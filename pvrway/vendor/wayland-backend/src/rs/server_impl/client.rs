    ...
    pub(crate) fn next_request(
        ...
        loop {
            ...
            let msg = match self.socket.read_one_message(|id, opcode| {
                ...
                Ok(msg) => msg,
                Err(error @ (MessageParseError::MissingData | MessageParseError::MissingFD)) => {
                    eprintln!("wayland backend incomplete request: {error:?}");
                    // need to read more data
                    if let Err(e) = self.socket.fill_incoming_buffers() {
                        eprintln!("wayland backend fill incoming failed: {e:?}");
                ...
                Err(error @ MessageParseError::Malformed) => {
                    eprintln!("wayland backend malformed request: {error:?}");
                    self.kill(DisconnectReason::ConnectionClosed);
