...
unsafe extern "C" {
    #[link_name = "recvmsg"]
    fn android_recvmsg(fd: c_int, message: *mut AndroidMsghdr, flags: c_int) -> isize;
}
...
#[cfg(target_os = "android")]
const fn cmsg_align(value: usize) -> usize {
...
impl Socket {
    ...
    pub fn rcv_msg(&self, buffer: &mut [u8], fds: &mut VecDeque<OwnedFd>) -> IoResult<usize> {
        #[cfg(target_os = "android")]
        {
            return self.rcv_msg_android(buffer, fds);
        }
        #[cfg(not(target_os = "android"))]
        {
        #[cfg(not(any(target_os = "macos", target_os = "redox", target_os = "android")))]
        let flags = RecvFlags::DONTWAIT | RecvFlags::CMSG_CLOEXEC;
        #[cfg(any(target_os = "macos", target_os = "redox", target_os = "android"))]
    ...
    #[cfg(target_os = "android")]
    fn rcv_msg_android(&self, buffer: &mut [u8], fds: &mut VecDeque<OwnedFd>) -> IoResult<usize> {
        let mut iov = AndroidIovec {
            base: buffer.as_mut_ptr().cast(),
            len: buffer.len(),
        };
        let control_len = cmsg_align(std::mem::size_of::<AndroidCmsghdr>())
            + cmsg_align(MAX_FDS_OUT * std::mem::size_of::<RawFd>());
        let mut control = vec![0_u8; control_len];
        let mut message = AndroidMsghdr {
        ...
        let bytes = unsafe {
            // SAFETY: self.stream is a live UnixStream and message points to writable buffers.
            android_recvmsg(self.stream.as_raw_fd(), &mut message, MSG_DONTWAIT)
        };
        if bytes < 0 {
            return Err(std::io::Error::last_os_error());
        }
        let header_size = std::mem::size_of::<AndroidCmsghdr>();
        let mut offset = 0_usize;
        while offset + header_size <= message.control_len {
            let header = unsafe {
                // SAFETY: offset is bounded by the control buffer allocated for recvmsg.
                &*(control.as_ptr().add(offset).cast::<AndroidCmsghdr>())
            };
            if header.len < header_size || offset + header.len > message.control_len {
                break;
            }
            if header.level == SOL_SOCKET && header.kind == SCM_RIGHTS {
                let data_offset = offset + cmsg_align(header_size);
                let data_len = header.len.saturating_sub(cmsg_align(header_size));
                let count = data_len / std::mem::size_of::<RawFd>();
                for index in 0..count {
                    let raw = unsafe {
                        // SAFETY: The kernel provided a complete SCM_RIGHTS payload.
                        *(control.as_ptr().add(data_offset + index * std::mem::size_of::<RawFd>()).cast::<RawFd>())
                    };
                    fds.push_back(unsafe {
                        // SAFETY: SCM_RIGHTS transfers ownership of each descriptor to this process.
